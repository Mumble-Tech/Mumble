...
export const AddContentButton = () => {
  ...
  const addContent = () => {
    ...
    if (validComponentList.includes(componentName)) {
      setContentState((oldContent: any) => [...oldContent, componentName]);
    } 
    ...
    <div className="add-content--container">
      {activeInput && (
        <div className="fixed">
          ...
          onChange={(e: any) => {setInnerText(e.target.value)}}/>
          <button className="add-content--submit" type="submit" onClick={addContent}>
            Submit
          </button>
        </div>
      )}
      ...
      {!activeInput && (
        <button className="add-content--btn" onClick={() => setActiveInput(true)}>
          Add Content
        </button>
      )}
    </div>
